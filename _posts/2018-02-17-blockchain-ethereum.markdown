...
It's not general purpose enough for everyday programming. But I like to think
about blockchain from that angle. Modern blockchains are limited but I hope that
after several iterations people will figure out how to implement distributed
resilient general-purpose high-performance virtual machine. And pure programmer
will be born.
...
* to satisfy previous property, each full node executes all scripts *(there are
  ideas to change this, but nothing was deployed so far)*.
...
We know that difficulty of mining puzzle is automatically adjusted by the system
based on previous blocks history. The goal of this adjustment is to make the
speed of blocks creation constant. Target speed is a one new block per 10
minutes. 10 minutes is a trade-off between a transactions finalization time and
a security.

Transaction finalization time is the time from request to its completion or a
time from the moment you made a payment and the time other person confirmed that
he/she received the payment and he/she pretty sure that the fact of payment will
never change. Common practice is to wait for 5 confirmations which means it's
about 50 minutes to finalize a transaction.

Security was mentioned because short block creation time increases forks
probability. The shorter block creation time the bigger probability of forks and
more orphaned blocks are produced *(an orphaned block is one which was rejected
in favor of longest branch)*. In order to understand why blocks creation time
affects forks probability, imagine how new blocks propagate through the p2p
network:
1. A full node receives new block, validates it and sends to other nodes. There
   is a transmission delay. And there is processing which happens between
...
Points 1 and 2 multiply and you obtain time required for propagation of a new
block. Two new blocks created during this interval will cause a fork. Now, let's
focus on another time interval: the time between blocks. If you reduce the time
required to mine the new block then you increase the probability of creation of
two new blocks within a given interval of time. As a result: you can't
infinitely reduce block propagation time because it includes networking hardware
latency and hence reducing the time between new blocks increases the probability
of forks.

Why are forks bad? First of all Bitcoin's 5 confirmations rule exists to
tolerate short living forks. Also, orphaned blocks don't contribute to the
network security because computational power used to create orphaned blocks is
literally wasted. This fact increases chances for an attacker to concentrate
enough computational power and to create the fork longer than 5 blocks *(and to
fool users who thought that 5 confirmations are secure enough for everyone)*. To
clarify this point: imagine that 40% of computational power is wasted to create
orphaned blocks. Then an attacker needs only 20+% of computational resources to
create 1/3 of useful computational power and to compete with good miners *(there
is the analysis which shows that 1/3 portion of computational power and some
luck is enough to create useful attacks on Bitcoin)*.
...
thing. However, ASICs cause centralization of mining hence is evil.
...
possible to mine with GPUs". They use memory hard puzzle which means solving
puzzle requires a lot of fast RAM memory. The key idea is that memory can't be
further specialized to work faster so ASICs should use ordinary memory. That
kills the idea of ASICs because commodity hardware with the same amount of RAM
will be cheaper than ASIC due to production volumes.
...
* how to ensure attackers can't leverage sharding for attacks?

Those are hard questions and Ethereum didn't answer them yet. The only thing I
understood from their wiki it that some sort of randomized shuffling can be used
