...
and to dig strait into altcoins. But it turns out that there are people with
technical background who heard about Bitcoin and probably even used it. But they
don't have enough time to read the [whole
book](http://chimera.labs.oreilly.com/books/1234000001802/index.html), taking
[online course](https://www.coursera.org/learn/cryptocurrency) or lurking
through different blogs which describe how crypto will disrupt you future
without many technical details.

This isn't hardcore technical article just light tough on how things work.
Please yourself familiar with article structure before reading. I failed to make
this post compact so it's possible to loose track of thoughts at some point.
...
work](#how-things-work) section. Probably it worth looking Highlights part of
each story.
...
![]({{ "/assets/2018-01-09-blockchain-bitcoin/light-full-miner.png" | absolute_url }})
...
Recall our classification from the previous post. State, State transformations,
Consensus. Let's start with state. We need to define what is stored and how it's
stored. Bitcoin uses several simple but clever
data structures, let's review them.
...
must be signed by the corresponding private key. The public key is used as an
...
computer. Soon we will discuss how to turn it into a distribute application.

### Mining

Mining helps solving two problems:
...
It takes two inputs: actual content and nonce field. Nonce field is just a dummy
field with arbitrary content. The Bitcoin puzzle is that: tweak nonce field
until you obtain a hash with N leading zero bits. The important properties of
that puzzle are:
* it's only solution is brute force;
* if two competitors A and B are solving the same puzzle then their chances to
  win the race are proportional to their computing power.
...
separate activities. To produce transaction you need a private key. To produce
block you need computational power and time to solve a puzzle. Generally, miners
take transactions from other bitcoiners and combine them into a new block.

Bitcoin uses really smart scheme for money distribution. I think it's the
smartest thing in the whole system. Each new block gives a fixed reword to a
...
rewords for a successful new block.

The process of blocks creation is called mining. Because it is hard and because
it produces new Bitcoins.
...
* People burn electricity *(i.e. real money)* to mine Bitcoins. I think it
  ...
  you are not going to sell it cheaper than your expenses was.
* Everyone who is willing to invest real money and to accept risks can
  participate in mining. It improves distribution. 
  
### Consensus

Finally, it's time to rock and to define what it's all about. We have talked
about linked list of blocks *(aka ledger, aka blockchain)* which represents a
history. We defined mining as a way to produce new blocks. Let's add few more
pieces and build the whole picture.

Bitcoin nodes form p2p network. Each node stores it's own copy of a transactions
history *(aka ledger)*. The shiny goal of a consensus algorithm is to make sure
that each node has the exactly same history. And it works like this:
...
4.  If there is no other candidate, then all nodes accept new block as the new
    true history.
...
6.  At some point, new blocks will be produced on top of either of two forks and
    the longer branch will become true history. But due to insanely bad luck,
    two new blocks can be produced approximately at the same time on top of both
    branches in the fork. This situation can't last for long time because low
    probabilities multiply. And hence longer forks have a lower probability of
    existence. Forks with length 3 are much less likely to happen than a fork
    with length 2 than a fork with length 1. Common practice in Bitcoin
    community is to believe that history deeper than 5 blocks will never change.
    In other words: transactions are finalized after 5 confirmations.

(\*) Puzzle difficulty is determined by previous blocks' timestamps written in a
blockchain. So existing history determines difficulty for the next block.

Why this algorithms is called a "Proof of work"? My intuition is that someone
who spent more computation power *(work)* can prove that his version of history
is the right one. In other words work is used as a proof.
...
confirmation, in the second block has 2 an so on. But there are transactions
which are not *(yet)* part of the history. Those are transactions with zero
confirmations. They are telegraphed by nodes and eventually they reach miners.
Miners produce blocks and after some time transactions become written into
history and they received confirmations. Set of unconfirmed transactions is
called a Memory Pool *(strange terminology)*.
...
A first transaction inside each block is called a coinbase transaction. It
specifies address where mining reward should go and it also contains additional
nonce field.

#### UTXO

Straitforward implementation of balances allows partial withdraw of money from
an account. This make makes light clients impossible because a system should
track all transactions to know the balance of each account.

Bitcoin improves situation by storing money inside unspent transaction outputs
(UTxOs). Each transaction has inputs and outputs. Output specifies the amount of
money it has and also specifies a link to its owner's account. Inputs are just
unspent outputs of other transactions. So you own money if you own some unspent
transaction outputs. Bitcoin requires transactions to completely spend each of
its input. This scheme simplifies computing of account's balance and still makes
possible partial withdraw from unspent output *(by producing 2 new outputs from
it where one of the new outputs is owned by your account)*.

![]({{ "/assets/2017-12-21-blockchain-intro/utxo.png" | absolute_url }})

The UTXO scheme greatly reduces amount of information required by light client
to compute it's balance.

#### Bloom filter

If a light client asks a full node "hey I want all transactions which affect
account XXX" then the full client can assume that the light client somehow
connected to that account. To protect it's privacy the light client can tell
...
scripts associated with taht transaction. Scripts allow implementing some more
complicated applications other than simple tokens movement. Examples are a
lottery, an escrow transactions, a voting. As far as I know, it's almost never
used in practice *(according to [this
...
Most interesting thing about bitcoin scripts is that they are replayed on all
full nodes. So in case of Bitcoin both storage and cpu are consumed on each node
...
1.  State.
    ...
    History of all transactions. Stored on all nodes. *There are also light clients.*
...
Things I intentionally skipped to make post shorter:
...
of mentioned topics in the following posts.
